...
import { loginresp, menupermission, menus, registerconfirm, resetpassword, roles, updatepassword, updateuser, usercred, userregister, users } from '../_model/user.model';
...
export class UserService {
  ...
  Userregisteration(_data: userregister) {
    return this.http.post(this.baseUrl + 'api/login/register', _data);
  }
  ...
  Proceedlogin(_data: usercred) {
    return this.http.post<loginresp>(this.baseUrl + 'api/login/login', _data);
  }
  ...
  Getallusers() {
    return this.http.get<users[]>(this.baseUrl + 'api/MedicinesLists');
  }
